...
| `header` | `(@Composable (`[`PdfPageInfo`]({{ site.baseurl }}/api/pdf-page-info)`) -> Unit)?` | `null` | Optional composable stamped at the top of every page, anchored ~18pt (0.25in) from the page edge, inside the page margin. Height is measured once (using a `pageCount = 2` sentinel so `if (pageCount > 1)` footers still reserve space) and is stable on every page; taller content is clipped. Body content keeps the configured margins unless the band (edge inset + height + a 10pt gap) exceeds the margin, in which case the body is pushed to fit. See [Headers and footers]({{ site.baseurl }}/usage/auto-pagination#headers-and-footers) |
| `footer` | `(@Composable (`[`PdfPageInfo`]({{ site.baseurl }}/api/pdf-page-info)`) -> Unit)?` | `null` | Optional composable stamped at the bottom of every page. Same placement/measurement rules as `header`. See [Headers and footers]({{ site.baseurl }}/usage/auto-pagination#headers-and-footers) |
