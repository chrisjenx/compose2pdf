...
6. **Fallback threshold**: the single-page check compares `measuredHeightPx <= effectiveContentHeightPx`. When it (or fillMaxHeight detection) triggers **and slots are present**, render a single page via the band path below — *not* via `renderVector(1)`/`addPage`, whose stretch-to-page geometry cannot express reserved bands.
